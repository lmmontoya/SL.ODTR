...
res_fun = function(results, DGP_fun, QAW_fun, kappa, desc) {
  ...
  psis = unlist(lapply(1:length(results), function(x) results[[x]]$EnYgRC["Psi_CV.TMLE",]))
  CIs = do.call('rbind', (lapply(1:length(results), function(x) c(CI1 = results[[x]]$EnYgRC["CI_CV.TMLE1",], CI2 = results[[x]]$EnYgRC["CI_CV.TMLE2",]))))
  bias = mean(psis) - truth
  var = var(psis)
  cov = mean(CIs[,"CI1"] < truth & truth < CIs[,"CI2"])
  da = unlist(lapply(1:length(results), function(x) results[[x]]$E0YgRC["E0YgRC.CVTMLE",]))
  cov_da = mean(CIs[,"CI1"] < da & da < CIs[,"CI2"])
  ...
  toreturn = data.frame(desc = desc, bias = bias, var = var, cov = cov, cov_da = cov_da, regret = mean_regret, tauP0vtauPn = mean_tauP0vtauPn)
  return(toreturn)
...
res_fun(results = EYgRC_allpos_discreteW_k0.1, kappa = 0.1, DGP_fun = DGP.rc.discreteW, QAW_fun = QAW.rc.allpos, desc = "All pos discrete W k = 0.1")
...
res_fun(results = EYgRC_allpos_contW_k0.1, kappa = 0.1, DGP_fun = DGP.rc.contW, QAW_fun = QAW.rc.allpos, desc = "All pos cont W k = 0.1")
...
res_fun(results = EYgRC_AL1_k0.1, kappa = 0.1, DGP_fun = DGP_bin_complex, QAW_fun = QAW_bin_complex, desc = "AL1 k = 0.1")
