...
<<echo = F, warning = F>>=
load(file = "/Users/lmontoya/Box/SL.ODTR/simulations/EYdopt/results/EYdopt_bin_simple.RData")
load(file = "/Users/lmontoya/Box/SL.ODTR/simulations/EYdopt/results/EYdopt_bin_medium.RData")
#load(file = "/Users/linamontoya/Box Sync/Dissertation/simulations/EYdopt/results/EYdopt_bin_medium_HAL.RData")
load(file = "/Users/lmontoya/Box/SL.ODTR/simulations/EYdopt/results/EYdopt_bin_most.RData")
...
<<echo = F, eval = T>>=
simple = make_table_fun(results = EYdopt_bin_simple)
medium = make_table_fun(results = EYdopt_bin_medium)
most = make_table_fun(results = EYdopt_bin_most)
@
...
#write.csv(table_EnYd0_for_E0Yd0, file = "/Users/lmontoya/Box/SL.ODTR/simulations/EYdopt/results/EnYd0_for_E0Yd0.csv", row.names = F)
@
...
write.csv(table_EnYdn_for_E0Yd0, file = "/Users/lmontoya/Box/SL.ODTR/simulations/EYdopt/results/EnYdn_for_E0Yd0.csv", row.names = F)
@
...
#write.csv(table_EnYdn_for_E0Ydn, file = "/Users/lmontoya/Box/SL.ODTR/simulations/EYdopt/results/EnYdn_for_E0Ydn.csv", row.names = F)
@
...
make_plot_fun = function(results, title) {
  ...
  EnYdn = do.call('rbind', lapply(results, function(x) x$EnYdn[grep("Psi_", rownames(x$EnYdn)), "EYd"]))
  colnames(EnYdn) = row.names(results[[1]]$EnYdn)[grep("Psi_", rownames(results[[1]]$EnYdn))]
  EnYdn = EnYdn[,-which(colnames(EnYdn) %in% c("Psi_unadj", "Psi_IPTW_DR"))]
  ...
  #EnYd0 = do.call('rbind', lapply(results, function(x) x$EnYd0[grep("Psi_", rownames(x$EnYd0)), "EYd"]))
  #colnames(EnYd0) = row.names(results[[1]]$EnYd0)[grep("Psi_", rownames(results[[1]]$EnYd0))]
  #EnYd0 = EnYd0[,-which(colnames(EnYd0) %in% c("Psi_unadj", "Psi_IPTW_DR"))]
  ...
  df = df_dn
  df$Estimator = factor(as.character(df$Estimator), labels = c("G-comp.", "IPTW", "TMLE", "CV-TMLE"), levels = c("Psi_gcomp", "Psi_IPTW", "Psi_TMLE", "Psi_CV.TMLE"))
...
#    geom_line(aes(y = 0, colour = "blue")) +
    ...
    scale_shape_discrete(name = "", labels = c("dn" = "Estimated optimal rule")) +
    scale_colour_manual(name = "         Causal parameter:",
                        values = c("black", "red"),
                        labels = c("red" = parse(text = TeX('$E\\[Y_{d^*_n}\\]$')),
                                   "black" = parse(text = TeX('$E\\[Y_{d^*}\\]$'))))+
    ylim(c(.38,.97)) +
...
plot1 = plot1 + ylab("Estimated value of estimated optimal rule")
plot2 = make_plot_fun(results = EYdopt_bin_medium, title = "ML + GLMs - moderately data adaptive")
...
#png(filename="/Users/lmontoya/Box/SL.ODTR/simulations/EYdopt/plotfig.png", width = 1300, height = 850)

ggarrange(plot1, plot3, ncol=2, nrow=1, common.legend = TRUE, legend="bottom")
