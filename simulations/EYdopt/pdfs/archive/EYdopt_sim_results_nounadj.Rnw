...
make_table_EYdopt = function(EYdopt, truevalues) {
  ...
  EYdopt = EYdopt[,-grep("unadj", colnames(EYdopt))]
  ...
  Bias = colMeans(Psi[,c("gcomp", "IPTW", "IPTW_DR", "LTMLE", "TMLE")] - EYdopt[,"EY0dn"])
  Bias = c(Bias, CV.TMLE = mean(Psi[,c("CV.TMLE")] - EYdopt[,"EY0dn_CVTMLE"]))
  Variance = diag(var(Psi[,c("gcomp", "IPTW", "IPTW_DR", "LTMLE", "TMLE", "CV.TMLE")]))
  MSE = colMeans((Psi[,c("gcomp", "IPTW", "IPTW_DR", "LTMLE", "TMLE")] - EYdopt[,"EY0dn"])^2)
...
make_plot_EYdopt = function(EYdopt, truevalues, title) {
  ...
  df$Estimator = factor(as.character(df$Estimator))
  df$Estimator = factor(as.character(df$Estimator), labels = c("G-comp.", "IPTW", "IPTW-DR", "TMLE", "CV-TMLE"), levels = c("gcomp", "IPTW", "IPTW_DR", "TMLE", "CV.TMLE"))
