...
trying = mean(QAW_bin_simple(A = d0, W))
Y_d0 = rbinom(10e7, 1, prob = QAW_bin_simple(A = d0, W))
truth = mean(Y_d0)

load("simulations/EYgstar/psi_glm_simple.RData")
alpha = alpha_psi_glm
c = c_psi_glm
dn = dn_psi_glm
...
table_simple = data.frame(table_simple[,1:2], sapply(table_simple[,3:5], round, 4))
...
table_complex = data.frame(Estimator = c(rep("TMLE", times = 3), rep("CV-TMLE", times = 3)),
                          ...
                          MSE = c(mse_EnYgstar_alpha, mse_EnYgstar_c, mse_EnYgstar_dn,
                                  mse_CV.EnYgstar_alpha, mse_CV.EnYgstar_c, mse_CV.EnYgstar_dn))
table_complex = data.frame(table_complex[,1:2], sapply(table_complex[,3:5], round, 4))
write.csv(table_complex, file = "simulations/EYgstar/table_complex.csv", )


alpha_complex = unlist(lapply(alpha, function(x) x$E0Ygstar["E0Ygstar.nonCVTMLE",])) - truth
c_complex = unlist(lapply(c, function(x) x$E0Ygstar["E0Ygstar.nonCVTMLE",])) - truth
dn_complex = unlist(lapply(dn, function(x) x$E0Ygstar["E0Ygstar.nonCVTMLE",])) - truth
...
load("simulations/true values/DGP_bin_complex_true_values.RData")
truth = DGP_bin_complex_true_values$EYd_star
...
class = rep(NA, times = 1000)
class[abs(unlist(lapply(c, function(x) x$E0Ygstar["E0Ygstar.nonCVTMLE",])) - truth) == abs(unlist(lapply(dn, function(x) x$E0Ygstar["E0Ygstar.nonCVTMLE",])) - truth)] = "eq"
class[abs(unlist(lapply(c, function(x) x$E0Ygstar["E0Ygstar.nonCVTMLE",])) - truth) > abs(unlist(lapply(dn, function(x) x$E0Ygstar["E0Ygstar.nonCVTMLE",])) - truth)] = "greater - bad"
class[abs(unlist(lapply(c, function(x) x$E0Ygstar["E0Ygstar.nonCVTMLE",])) - truth) < abs(unlist(lapply(dn, function(x) x$E0Ygstar["E0Ygstar.nonCVTMLE",])) - truth)] = "less than - good"
table(class)
abs(unlist(lapply(c, function(x) x$E0Ygstar["E0Ygstar.nonCVTMLE",])) - truth) <= abs(unlist(lapply(dn, function(x) x$E0Ygstar["E0Ygstar.nonCVTMLE",])) - truth)
